...
class LogConfig(BaseSettings):
    ...
    LOG_FORMAT: str = (
        "%(asctime)s %(levelname)-2s [%(name)s] [%(filename)s:%(lineno)d] - %(message)s"  # 日志格式
    )
...
def setup_logging(
    app: Optional[FastAPI] = None, config: Optional[LogConfig] = None
) -> None:
    """
    设置 FastAPI 应用的日志系统
    ...
    # 创建文件处理器（按大小分割），添加 encoding 参数
    file_handler = RotatingFileHandler(
        os.path.join(config.LOG_DIR, "app.log"),
        maxBytes=config.LOG_FILE_MAX_SIZE,
        backupCount=config.LOG_FILE_BACKUP_COUNT,
        encoding="utf-8",  # 新增：显式指定 UTF-8 编码
    )
...
def setup_celery_logging(config: Optional[LogConfig] = None) -> None:
    ...
    # 创建Celery文件处理器（按大小分割），添加 encoding 参数
